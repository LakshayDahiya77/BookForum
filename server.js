...
const HuggingFaceClient = require("./huggingFaceClient"); // Adjust the path as necessary
...
app.post("/book-summarizer", async (req, res) => {
  try {
    ...
    const summary = await hfClient.summarize(prompt);
  ...
  async (req, res) => {
    try {
      ...
      const fileUpload = await bucket.upload(file.path, {
        destination: `profile_pics/${file.filename}`,
      });
      ...
      user.profilePicURL = downloadURL[0];
      await user.save();
