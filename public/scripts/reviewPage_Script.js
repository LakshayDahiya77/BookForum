...
const reviewSummarizerContainer = document.querySelector(
  ".review-summarizer-section"
);
...
summarizeButton.addEventListener("click", async () => {
  ...
  const reviews = Array.from(reviewTextElements).map((element) =>
...
window.addEventListener("scroll", function () {
  ...
  if (window.pageYOffset > bookSummary.offsetTop) {
    ...
    document.getElementById("floating-header-text").innerText =
...
document.addEventListener("DOMContentLoaded", () => {
  const likesToolbarContainers = document.querySelectorAll(
    ".likes-toolbar-container"
  );
  likesToolbarContainers.forEach((container) => {
    container.addEventListener("click", async () => {
      const reviewId = container
        .querySelector(".heart-icon")
        .getAttribute("data-review-id");
      const response = await fetch(`/reviews/like/${reviewId}`, {
        method: "POST",
      });
      if (response.ok) {
...
editBookButton.addEventListener("click", () => {
  ...
  document.getElementById("newBookName").value = document
    .getElementById("book-title")
    .textContent.trim();
  document.getElementById("newBookAuthors").value = document
    .getElementById("book-author")
    .textContent.trim();
  document.getElementById("newBookTagline").value = document
    .getElementById("book-tagline")
    .textContent.trim();
  document.getElementById("newPublishedYear").value = document
    .getElementById("book-published-year-value")
    .textContent.trim();
  const currGenres = document
    .getElementById("book-genre-values")
    .textContent.trim()
    .trim()
    .split(", ");
